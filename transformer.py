...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        self.pos_table = torch.FloatTensor(
            pos_table).cpu()  # enc_inputs: [seq_len, d_model]

    def forward(self, enc_inputs):  # enc_inputs: [batch_size, seq_len, d_model]
        enc_inputs += self.pos_table[:enc_inputs.size(1), :]
        return self.dropout(enc_inputs.cpu())


def get_attn_pad_mask(seq_q,
                      seq_k):  # seq_q: [batch_size, seq_len] ,seq_k: [batch_size, seq_len]
    batch_size, len_q = seq_q.size()
    batch_size, len_k = seq_k.size()
    pad_attn_mask = seq_k.data.eq(0).unsqueeze(
        1)  # 判断 输入那些含有P(=0),用1标记 ,[batch_size, 1, len_k]
...
def get_attn_subsequence_mask(seq):  # seq: [batch_size, tgt_len]
    ...
    subsequence_mask = np.triu(np.ones(attn_shape),
                               k=1)  # 生成上三角矩阵,[batch_size, tgt_len, tgt_len]
    subsequence_mask = torch.from_numpy(
        subsequence_mask).byte()  # [batch_size, tgt_len, tgt_len]
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V,
                attn_mask):  # Q: [batch_size, n_heads, len_q, d_k]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V,
                attn_mask):  # input_Q: [batch_size, len_q, d_model]
        ...
        return nn.LayerNorm(d_model).cpu()(output + residual), attn
...
class PoswiseFeedForwardNet(nn.Module):
    ...
    def forward(self, inputs):  # inputs: [batch_size, seq_len, d_model]
        ...
        return nn.LayerNorm(d_model).cpu()(
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs,
                enc_self_attn_mask):  # enc_inputs: [batch_size, src_len, d_model]
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, dec_self_attn_mask,
        ...
        dec_outputs = self.pos_ffn(
            dec_outputs)  # dec_outputs: [batch_size, tgt_len, d_model]
...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs,
                dec_inputs):  # enc_inputs: [batch_size, src_len]
        # dec_inputs: [batch_size, tgt_len]
        enc_outputs, enc_self_attns = self.Encoder(
            enc_inputs)  # enc_outputs: [batch_size, src_len, d_model],
        # enc_self_attns: [n_layers, batch_size, n_heads, src_len, src_len]
        dec_outputs, dec_self_attns, dec_enc_attns = self.Decoder(
            dec_inputs, enc_inputs,
            enc_outputs)  # dec_outpus    : [batch_size, tgt_len, d_model],
        # dec_self_attns: [n_layers, batch_size, n_heads, tgt_len, tgt_len],
        # dec_enc_attn  : [n_layers, batch_size, tgt_len, src_len]
        dec_logits = self.projection(
            dec_outputs)  # dec_logits: [batch_size, tgt_len, tgt_vocab_size]
        return dec_logits.view(-1, dec_logits.size(
